...
def draw_corners(
    img: np.ndarray, corner_2d: np.ndarray,
    ...
    for start_idx, end_idx in indexes:
        result = cv2.line(
            img=result,
            pt1=tuple([int(val) for val in corner_data[start_idx]]),
            pt2=tuple([int(val) for val in corner_data[end_idx]]),
...
def draw_pts2d(
    img: np.ndarray, pts2d: np.ndarray,
    color: tuple = (255, 0, 0), radius: int = 2
) -> np.ndarray:
    result = img.copy()
